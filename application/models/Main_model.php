...
class Main_model extends CI_Model
{
  ...
  public function update_progress($id)
  {
    ...
    if (!$this->upload->do_upload('fileupdate')) {
      $this->session->set_flashdata('status', 'Something went wrong! File lampiran lebih dari 2MB atau format tidak didukung.');
  ...
  public function emailbuatticket($id)  //email akan dikirim ke admin ketika user membuat request baru
  {
    //Query untuk mendapatkan data detail dari setiap ticket
    ...
    $isiEmail = file_get_contents(APPPATH . "template/template_buat_email.php");

    $priorityBadge = '';
    $prioritas = strtolower($query->nama_prioritas);

    if ($prioritas == 'high') {
      $priorityBadge = '<span style="display: inline-block; padding: 4px 8px; background-color: #B14145; color: #B14145; border-radius: 12px; font-size: 12px; font-weight: bold;">' . $query->nama_prioritas . '</span>';
    } elseif ($prioritas == 'medium') {
      $priorityBadge = '<span style="display: inline-block; padding: 4px 8px; background-color: #FC8500; color: #FC8500; border-radius: 12px; font-size: 12px; font-weight: bold;">' . $query->nama_prioritas . '</span>';
    } else {
    ...
    $replacements = array(
      ...
      '{SUB_KATEGORI}' => $query->nama_sub_kategori,
      '{PRIORITY_BADGE}' => $priorityBadge,
      '{PRIORITAS}' => $prioritas,
      '{TANGGAL}' => date('d-m-Y', strtotime($query->tanggal)),
      '{LOKASI}' => $query->lokasi,
      '{PROBLEM_SUMMARY}' => $query->problem_summary,
      '{PROBLEM_DETAIL}' => nl2br($query->problem_detail),
      '{TRACKING_URL}' => $trackingUrl
    );

    $isiEmail = str_replace(array_keys($replacements), array_values($replacements), $isiEmail);


    $trackingUrl = base_url('ticket_user/detail/' . $query->id_ticket);
    ...
    $this->email->to($to);
    $this->email->subject("Confirmation: Ticket  $query->id_ticket Has Been Submitted");
  ...
  public function emailapprove($id)
  {
    ...
    $isiEmail = file_get_contents(APPPATH . "template/template_approve_to_user.php");
  ...
  public function emailreject($id)
  {
    ...
    $isiEmail = file_get_contents(APPPATH . "template/template_reject_to_user.php");

    $priorityBadge = '';
    $prioritas = strtolower($query->nama_prioritas);

    if ($prioritas == 'high') {
      $priorityBadge = '<span style="display: inline-block; padding: 4px 8px; background-color: #EF4444; color: #7F1D1D; border-radius: 12px; font-size: 12px; font-weight: bold;">' . $query->nama_prioritas . '</span>';
    } elseif ($prioritas == 'medium') {
      $priorityBadge = '<span style="display: inline-block; padding: 4px 8px; background-color: #F59E0B; color: #78350F; border-radius: 12px; font-size: 12px; font-weight: bold;">' . $query->nama_prioritas . '</span>';
    } else {
      $priorityBadge = '<span style="display: inline-block; padding: 4px 8px; background-color: #10B981; color: #064E3B; border-radius: 12px; font-size: 12px; font-weight: bold;">' . $query->nama_prioritas . '</span>';
    }
    ...
    $replacements = array(
      ...
      '{SUB_KATEGORI}' => $query->nama_sub_kategori,
      '{PRIORITY_BADGE}' => $priorityBadge,
      '{PRIORITAS}' => $query->nama_prioritas,
      '{LOKASI}' => $query->lokasi,
      '{TANGGAL}' => date('d-m-Y', strtotime($query->tanggal)),
      '{PROBLEM_SUMMARY}' => $query->problem_summary,
      '{PROBLEM_DETAIL}' => nl2br($query->problem_detail),
  ...
  public function emailtugas($id) //email akan masuk ketika di approve admin dan ditugaskan dan dikirimkan ke email teknisi teknisi 
  {
    //Query untuk mendapatkan data detail dari setiap ticket
    $query = $this->db->query("SELECT A.id_ticket, A.status, A.progress, A.tanggal, A.tanggal_proses, A.tanggal_solved, A.problem_summary, A.problem_detail, A.filefoto, B.nama_sub_kategori, C.id_kategori, C.nama_kategori, D.nama, D.email, D.telp, F.nama AS nama_teknisi, G.lokasi, H.nama_bagian_dept, I.nama_dept, J.nama_prioritas FROM ticket A 
    LEFT JOIN kategori_sub B ON B.id_sub_kategori = A.id_sub_kategori
    ...
    $isiEmail = file_get_contents(APPPATH . "template/template_to_technician.php");
    ...
    $replacements = array(
      '{ID_TICKET}' => $query->id_ticket,
      '{NAMA}' => $query->nama,
      '{NAMA_TEKNISI}' => $query->nama_teknisi,
      '{EMAIL}' => $query->email,
      '{KATEGORI}' => $query->nama_kategori,
      '{SUB_KATEGORI}' => $query->nama_sub_kategori,
      '{PRIORITY_BADGE}' => $priorityBadge,
  ...
  //ketika di acc teknisi, akan dikirimi email kembali ke user
...

  {
    ...
    $isiEmail = file_get_contents(APPPATH . "template/template_tiket_onprocess_to_user.php");
    ...
    $replacements = array(
      ...
      '{TANGGAL}' => date('d-m-Y', strtotime($query->tanggal)),
      '{TANGGAL_PROSES}' => $query->tanggal_proses,
      '{TANGGAL_SOLVED}' => date('d-m-Y', strtotime($query->tanggal_solved)),
      '{LOKASI}' => $query->lokasi,
      '{PROBLEM_SUMMARY}' => $query->problem_summary,
      '{PROBLEM_DETAIL}' => nl2br($query->problem_detail),
      '{TRACKING_URL}' => $trackingUrl,
  ...
  public function emailselesai($id) //ini email ketika sudah 100% atau selesai
  {
    //Query untuk mendapatkan data detail dari setiap ticket
    $query = $this->db->query("SELECT A.id_ticket, A.status, A.progress, A.tanggal, A.tanggal_proses, A.tanggal_solved, A.problem_summary, A.problem_detail, A.filefoto, B.nama_sub_kategori, C.id_kategori, C.nama_kategori, D.nama, D.email, D.telp, F.nama AS nama_teknisi, G.lokasi, H.nama_bagian_dept, I.nama_dept, J.nama_prioritas FROM ticket A 
    LEFT JOIN kategori_sub B ON B.id_sub_kategori = A.id_sub_kategori
    ...
    if ($query->progress > 0 && $query->progress < 100) {
      $isiEmail = file_get_contents(APPPATH . "template/template_tiket_ongoing.php");
      ...
      $isiEmail = file_get_contents(APPPATH . "template/template_tiket_selesai.php");
      ...
      $replacements = array(
        ...
        '{TANGGAL}' => date('d-m-Y', strtotime($query->tanggal)),
        '{TANGGAL_PROSES}' => $query->tanggal_proses,
        '{TANGGAL_SOLVED}' => $query->tanggal_solved,
        '{LOKASI}' => $query->lokasi,
        '{PROBLEM_SUMMARY}' => $query->problem_summary,
        '{PROBLEM_DETAIL}' => nl2br($query->problem_detail),
        '{TEKNISI}' => $query->nama_teknisi,
