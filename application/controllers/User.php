...
class User extends CI_Controller
{
	...
	public function tambah()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$insertDB = $this->db->insert('user', $data);
			if(!$insertDB){
				echo "<pre>";
				echo "Gagal meng-insert data ke database!\n";
				echo "Error database: ";
				print_r($this->db->error()); // Ini sangat penting!
				echo "\nQuery terakhir: " . $this->db->last_query(); // Juga sangat penting!
				echo "</pre>";
				die("Proses dihentikan karena insert gagal."); // Hentikan e
			}
			if (!$this->email->send()) {
